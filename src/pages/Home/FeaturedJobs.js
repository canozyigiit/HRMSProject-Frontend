					...
					jobAdverts.slice(0, 4).map((jobAdvert)=>(
						...
						<div className="fj_post">
							<div className="details">
							 ...
							 <img  src={jobAdvert.employerPhoto} alt=" "/> 
                                        </div> 
								<h4>{jobAdvert.jobPositionName}</h4>
								<p>Yayın Tarihi:  {new Date(jobAdvert.createdDate).toDateString()}  / Son Başvuru Tarihi: {new Date(jobAdvert.deadLine).toDateString()} <a href=" " className="text-thm" > Şirket :{jobAdvert.employerCompanyName}</a></p>
